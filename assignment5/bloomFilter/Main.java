/******************************************************************
 *
 *   Bloom Filter Exercise - https://www.javamex.com/tutorials/collections/bloom_filter_java.shtml
 *
 *   Additional comments provided throughout source code for educational purposes
 *
 ********************************************************************/
 ...
 * Bloom Filters
 *
 * A Bloom filter is an implementation of a set which allows a certain probability of
 * 'false positives' when determining if a given object is a member of that set, in return
 * for a reduction in the amount of memory required for the set. It effectively works as
 * follows:
 *      1) we allocate 'm' bits to represent the set data.
 *      2) We provide a hash function, which, instead of a single hash code, produces
 *         'k' hash codes and sets those bits.
 *      3) To add an element to the set, we derive bit indexes from all 'k' hash codes and
 *         set those bits.
 *      4) To determine if an element is in the set, we again calculate the corresponding
 *         hash codes and bit indexes, and say it is likely present if and only if all
 *         corresponding bits are set.
 *
 * The margin of error (or false positive rate) thus comes from the fact that as we add
 * more and more objects to the set, we increase the likelihood of "accidentally" setting
 * a combination of bits that corresponds to an element that isn't actually in the set.
 * However, through tuning the bloom filter setup based on the expected data, we mathematically
 * have control over the desired false positive probability rate that we want to received
 * based on probability theory.
 *
 * False Positive rate discussion:
 *
 * The Bloom filter performance changes as we change parameters discussed below with the
 * class constructors. There are two key variables that impact the false positive rate:
 *     1) number of bits per item
 *     2) number of hash codes
 *
 * In other words, how many more bits are there in the filter than the maximum number of
 * items we want to represent in the set, and hence the number of bits that we actually
 * set for each element that we add to the set. The more bits we require to be marked as
 * set to '1' in order to mark an element as 'present' - e.g., the more hash code per item -
 * the lower the chance of false positives, because for a given element potentially in
 * the set, there's less chance of some random combination of bits from other elements
 * also accidentally marking that element as present when it isn't.
 *
 * But, for a given bit filter size, there is a 'point of no return', at which having
 * more hash codes simply means that we fill up the bit set too quickly as we add
 * elements -- and hence get more false positives -- than with fewer hash codes.
 *
 * Based on this discussion, you can find many Bloom Filter calculators available online
 * to determine how to adjust the variables inorder to achieve the desired probability of
 * false positive rates that you can tolerate and/or desire for your application, e.g.,:
...
class BloomFilter {
     ...
     * of hashCode() — don't require the goals above to be met. Java maps and sets,
     * rather than storing just the hash code, store the actual key object.
     ...
     *
     * Below implements a 64-bit Linear Congruential Generator (LCG). It uses a table
     * of 256 random values indexed by successive bytes in the data, and recommends a
     * multiple suitable for an LCG with a modulus of 2^64 (which is effectively what
     * we have when we multiple using 64-bit long.
     *
     * The value of HMULT is found to be a good practice with 64-bit LCG. It has roughly
     * half of its bits set and is 'virtually' prime (it is composed of three prime
     * factors). The value of HSTART is arbitrary, essentially any value would do.
     *
    ...
    public boolean contains(String s) {
      ...
      for(int i = 0; i < noHashes; i++){
        long hash = hashCode(s, i);
        int bitNo = (int) (hash) & this.hashMask;
        if(!data.get(bitNo)){
          return false;
        }
      }
...
public class Main {
    public static void main(String[] args) {
         ...
         *
         * The test will loop through hash map sizes between 2^14 through 2^23, and
         * for each hash map size, will run through tests using 1 to 8 hash codes
         * per instantiation of the bloom filter. Each instantiation will be populated
         * with 16,384 random strings, and the test will probe the bloom filter
         * instantiation (a.k.a., invoke methods contains() using random strings)
         * 2,500,000 times. The false positive rate will be displayed for each
        ...
        for (int log2bits = 14; log2bits <= 23; log2bits++) {
            for (int noHashes = 1; noHashes <= 8; noHashes++) {
                ...
                System.out.println("\nBloom filter populated with " + noItems + " elements with map size of "
                                   + (int) Math.pow(2,log2bits) + " and " + noHashes + " hash codes");
                System.out.print("\tCombination's false positive ratio is: " + falsePositiveRate);
                if ( falsePositiveRate == 0 ) System.out.print(" - no false positives on this combination!");
                System.out.println();
            }
        }
    }
